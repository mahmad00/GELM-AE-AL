...
%% Warning off
warning('off', 'all');
%% Inital Number of Training Samples Per Class and In Total
N = 5;                 %% Samples per Class
Nall = 30;             %% Total Samples
%% Pick Samples
% Samples = {'Classified', 'Misclassified'};
...
load('SalinasA_gt');
gt = salinasA_gt;
map = [1 1; 10 2; 11 3; 12 4; 13 5; 14 6];
[~, loc] = ismember(gt, map(:,1));
gt = loc;
folder = sprintf('%s/SalinasA', pwd);
if ~exist(folder, 'dir')
    mkdir(folder);
end
load('SalinasA_corrected');
img = salinasA_corrected; clear map loc salinas*
%% ELM Tuning Parameters
ActivationFunction = 'radbas'; %% sig, radbas
rhoValue = 0.05;
sigpara = [0.7, 6];
sigpara1 = [0.8, 4];
TotalLayers = 5;
C = [1e-1, 1e4, 1e8, 1e10, 1e12];
[nodes, ~] = size(img(:,2));
HiddenNeurons = [nodes, 5000];
Parameters = struct('HNs', HiddenNeurons, 'Regu', C, 'Rho', ...
    rhoValue, 'Sigpara', sigpara, 'sigpara1', sigpara1, 'AF',...
        ActivationFunction, 'TLs', TotalLayers);
%% Reshape and Normalize
[img, ~] = Res_Nor(img);
%% Initial Training and Test Samples
[TrC, TeC] = Initial_Tr_Te(gt, N, Nall);
%% Active Learning Parameters
Candidate_Set = TeC;
M = 600;                %% Total Number of Samples to Select
h = 30;                 %% Number of Samples Per Iteration
AL_Strtucture = struct('Candidate_Set', Candidate_Set, 'M', M, 'h', h);
...
saveas(figure(1), [folder '/', 'Accuracy.png']);
pause(1)
close all;
...
ax_height = outerpos(4) - ti(2) - ti(4);
ax.Position = [left bottom ax_width ax_height];
saveas(figure(1), [folder '/', 'Time.png']);
pause(1)
close all; clear;
%% Internal Functions
...
function [TrC, TeC] = Initial_Tr_Te(gt, n, nall)
A = find(gt);
Pixels = gt(A);
y = [A Pixels]; clear Pixels A
y = sortrows(y, 2);
y = y';
...
indexes = indexes(:);
%% Train and Test Class and Their Indexes
indexes = indexes';
TrC = y(:, indexes);
TeC = y;
TeC(:, indexes) = [];
...
function [img, bands] = Res_Nor(img)
[rows,cols,bands] = size(img);
norm = mean(reshape(sqrt(sum(img.^2,3)),rows*cols,1));
img = img./norm;
img = reshape(img,[rows*cols bands]); clear norm 
end